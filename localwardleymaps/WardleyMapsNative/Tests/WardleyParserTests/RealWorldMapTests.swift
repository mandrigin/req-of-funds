...
@Suite("Real World Maps")
struct RealWorldMapTests {
    let parser = WardleyParser()

    @Test("Parses defence doctrine map (Simon Wardley)")
    func testDefenceDoctrine() {
        let input = """
        ...
        // Title
        #expect(map.title == "defence - military doctrine")

        // Style
        #expect(map.presentation.style == "wardley")

        // Evolution labels (custom)
        #expect(map.evolution.count == 4)
        #expect(map.evolution[0].line1 == "genesis / concept")

        // Notes
        #expect(map.notes.count == 8)

        // Components — should have many
        #expect(map.elements.count > 30)

        // Pipelines
        #expect(map.pipelines.count > 5)

        // Links
        #expect(map.links.count > 20)

        // No parse errors
        #expect(map.errors.isEmpty)
    }

    @Test("Parses map with all feature types")
    func testFullFeatureMap() {
        let input = """
        title Full Feature Map
        evolution Custom->Transitional->Product->Commodity
        style colour

        anchor Business [0.95, 0.63]
        component Platform [0.65, 0.55] label [-30, -15] (buy)
        component API [0.45, 0.72]
        component Database [0.25, 0.85]
        evolve Database 0.92 label [10, -5]
        pipeline Platform [0.40, 0.70]
        ...
        annotation 1 [[0.50,0.30],[0.70,0.60]] Platform needs modernizing
        annotations [0.80, 0.05]

        note +Critical path [0.15, 0.45]

        pioneers [0.90, 0.05, 0.70, 0.30]
        settlers [0.70, 0.30, 0.40, 0.60]
        townplanners [0.40, 0.60, 0.10, 0.95]

        buy Platform
        build API

        accelerator FastTrack [0.55, 0.80]
        deaccelerator LegacyBlock [0.35, 0.40]

        url docs [https://docs.example.com]
        submap External [0.30, 0.60] url(https://example.com)
        """
        let map = parser.parse(input)

        #expect(map.title == "Full Feature Map")
        #expect(map.presentation.style == "colour")
        #expect(map.evolution[0].line1 == "Custom")
        ...
        #expect(map.attitudes.count == 3)
        #expect(map.methods.count == 2)
        #expect(map.accelerators.count == 2)
